...
export class EnhancedApiService {
        ...
        this.axiosInstance.interceptors.request.use(
            (config) => {
                try {
                    ...
                    // Add tenant slug from URL path to headers
    ...
    private processSuccessResponse(response: AxiosResponse): AxiosResponse {
        ...
        // Always attempt refinement to handle pattern like { data: { data: [], total: X } }
        response.data = refineApiResponse(response.data);
            ...
            try {
                ...
                try {
                    ...
                    if (token) {
                        ...
                        // We use this.get directly here to avoid potential circular dependencies
                        // and to pass skipAuthRefresh to avoid infinite 401 loops
                        const user = await this.get('/auth/profile', {skipAuthRefresh: true});

                        if (user) {
                            // Map user data safely
                            const mappedUser = mapUser(user);
                            
                            // Attempt to update auth state via nuxtApp context if possible
                            // otherwise we just proceed with the refreshed token
                            try {
                                if (this.nuxtApp?.$pinia) {
                                    // We can't easily use the store here without circularity 
                                    // but we'll try to at least keep the token valid
                                }
                            } catch (e) {
                            ...
                            throw new Error('User data not returned after refresh');
                        }
                    ...
                    // Ensure we are in a client environment before redirecting
                    if (import.meta.client) {
